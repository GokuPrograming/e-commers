...
const Cart: React.FC = () => {
    ...
    const fetchTotalCar = async () => {
      try {
        ...
        if (token) {
          ...
          const apiUrl =
            "https://api-cuchau-store-pg.onrender.com/user/car/total";
  ...
  const removeFromCart = async (id_producto: number): Promise<void> => {
    ...
    setUserId(userIdFromToken);
    const apiUrl =
      "https://api-cuchau-store-pg.onrender.com/user/car/Delete_element";
    try {
      ...
      if (response.ok) {
        ...
        setCartItems(
          cartTotal.filter((item) => item.id_producto !== id_producto)
        );
      } else {
        // Manejo de errores en caso de respuesta no exitosa
        console.error(
          `Error al eliminar el producto ${id_producto}. Código de estado: ${response.status}`
        );
      }
    } catch (error) {
              ...
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-4">
                    <Image
                      src={item.imagen || "/placeholder_image.png"}
                      width={600}
                      height={600}
                      alt={""}
